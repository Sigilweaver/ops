...
- **Chromatograms are unwired end-to-end.** `ChromatogramRecord`/
  ...
  - OpenSZRaw - no new issue; already-open [OpenSZRaw#2](https://github.com/Sigilweaver/OpenSZRaw/issues/2) (PDA/LSS chromatogram payload decode) already scopes the `iter_chromatograms` wiring as part of its done-criteria. Still unsolved after 14 combined investigation sessions (2026-07-19/20, two contributors) - width selection is known to be signal-magnitude-driven with real but unexploited temporal correlation between segments, see `docs/format/04-lcd-chromatogram-pda.md`'s factsheet for the full ruled-out list. Two follow-up issues spun out 2026-07-20: [OpenSZRaw#20](https://github.com/Sigilweaver/OpenSZRaw/issues/20) (widen the corpus for a non-empty `LSS Raw Data` sample - the literally-named stream is 0 bytes in every locally available file) and [OpenSZRaw#21](https://github.com/Sigilweaver/OpenSZRaw/issues/21) (decode `LC Raw Data/Chromatogram Ch5`/`Ch6` instead/first - real, populated, structurally simpler than PDA's per-wavelength stream).
